...
export class BeiGe extends TriggerSkill {
  ...
  public async onEffect(room: Room, skillUseEvent: ServerEventFinder<GameEventIdentifiers.SkillEffectEvent>) {
    ...
    await room.dropCards(CardMoveReason.SelfDrop, skillUseEvent.cardIds!, skillUseEvent.fromId, skillUseEvent.fromId, this.Name);
    const { toId , fromId , cardIds } = triggeredOnEvent as ServerEventFinder<GameEventIdentifiers.DamageEvent>;
    if ((fromId !== undefined) && (Sanguosha.getCardById(cardIds![0]).GeneralName === 'slash')) {
      ...
      if (judgeCard.Suit === CardSuit.Club) {
        const response = await room.askForCardDrop(fromId, 2, [PlayerCardsArea.HandArea, PlayerCardsArea.EquipArea], true, undefined, this.Name);
        ...
        await room.recover({ recoveredHp: damageEvent.damage, recoverBy: toId, toId: toId!, triggeredBySkills: [this.Name] });
