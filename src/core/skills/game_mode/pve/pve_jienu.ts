...
// 难3 【介怒】锁定技，当你翻面时，你回复一点体力并对所有其他角色造成两点火属性伤害；出牌阶段开始时或受到伤害后，若你体力小于已损失体力，则你翻面。
...
export class PveJieNu extends TriggerSkill {
  ...
  async onEffect(room: Room, event: ServerEventFinder<GameEventIdentifiers.SkillEffectEvent>) {
    ...
    if (identifier === GameEventIdentifiers.DamageEvent || identifier === GameEventIdentifiers.PhaseStageChangeEvent) {
      if (room.getPlayerById(fromId).Hp < room.getPlayerById(fromId).LostHp) {
